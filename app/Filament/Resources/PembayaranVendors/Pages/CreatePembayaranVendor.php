...
class CreatePembayaranVendor extends CreateRecord
{
    protected static string $resource = PembayaranVendorResource::class;

    protected static ?string $title = 'Tambah Pembayaran Vendor';

    protected ?string $heading = 'Tambah Pembayaran Vendor';
    ...
    protected function afterCreate(): void
    {
        ...
        DB::transaction(function () use ($pembayaran) {
            ...
            if (! $pembayaran->piutang_id) {
                throw new \Exception(
                    'Pembayaran hanya untuk transaksi kredit (piutang).'
                );
            }
            ...
            $diskonTermin = (float) (
                $pembayaran->diskon_termin ?? 0
            );

            $nominalBayarTerms = (float) (
                $pembayaran->total_setelah_diskon
                ?? $nominalBayar
            );
